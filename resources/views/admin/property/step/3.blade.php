        ...
        <div class="card bl--5 border--warning">
            <div class="card-body">
                <p class="text--warning">
                    ...
                    <a href="{{ route('admin.seo') }}">@lang('System Setting > SEO Configuration').</a>
                </p>
            </div>
        </div>
    </div>
        ...
        <div class="row">
            <div class="col-xl-4">
                <div class="form-group">
                    ...
                    <x-image-uploader class="w-100" :imagePath="getImage(getFilePath('seo') . '/' . @$property->seo_content->image, getFileSize('seo'))" :size="getFileSize('seo')" :required="false" name="image" />
                </div>
            </div>
            <div class="col-xl-8 mt-xl-0 mt-4">
                <div class="form-group">
                    ...
                    <select name="keywords[]" class="form-control select2-auto-tokenize" multiple="multiple">
                        @if (@$property->seo_content->keywords)
                            @foreach ($property->seo_content->keywords as $option)
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" rows="3" class="form-control">{{ @$property->seo_content->description }}</textarea>
                </div>
                <div class="form-group">
                    <label>@lang('Social Title')</label>
                    <input type="text" class="form-control" name="social_title"
                        value="{{ @$property->seo_content->social_title }}" />
                </div>
                <div class="form-group">
                    <label>@lang('Social Description')</label>
                    <textarea name="social_description" rows="3" class="form-control">{{ @$property->seo_content->social_description }}</textarea>
                </div>
            </div>
        </div>
    </div>
