<div class="container mt-5">
    ...
    <div class="form-group mb-3">
        <label for="distribution_waterfall">Distribution waterfall to use</label>
        <select id="distribution_waterfall" name="distribution_waterfall" class="form-control"
            x-on:input="distributionErrors.distribution_waterfall =''" x-model="distributionForm.distribution_waterfall">
            <option value="" selected disabled>Select waterfall</option>
            @foreach ($deal->waterfalls as $waterfall)
                <option value="{{ $waterfall->id }}">{{ $waterfall->waterfall_name }}</option>
            @endforeach
            <option value="new_waterfall">Add new waterfall</option>
        </select>
        <span x-show="distributionErrors.distribution_waterfall" x-text="distributionErrors.distribution_waterfall"
            class="text-danger"></span>
    </div>

    <div class="form-group mb-3">
        <label for="distribution_source">Source <span class="text-danger">*</span></label>
        <select class="form-control" id="distribution_source" x-on:input="distributionErrors.source =''"
            x-model="distributionForm.source">
    ...
    <div class="form-group mb-3">
        ...
        <select class="form-control" id="deduct_from" x-model="distributionForm.count_toward">
            <option value="no_applicable">None/not aplicable</option>
    ...
    <div class="form-group mb-3">
        <label for="distribution_amount">Distribution amount <span class="text-danger">*</span>($)</label>
        <input type="text" x-on:input="moneyFormat($el)" class="form-control" id="distribution_amount"
            x-model="distributionForm.amount">
    </div>
    <div class="form-group mb-3">
        ...
        <div class="container" x-data="alpineMuliSelect({ selected: [], elementId: 'multSelect' })">
            ...
            <select class="d-none" id="multSelect" @change="selectChange()">
                @foreach ($deal->classes as $class)
                    <option value="{{ $class->id }}" data-search="{{ $class->equity_class_name }}">
                        {{ $class->equity_class_name }}</option>
                @endforeach
                @foreach ($deal->buckets as $bucket)
                    @foreach ($bucket->classes as $bclass)
                        <option value="{{ $bclass->id }}" data-search="{{ $class->equity_class_name }}"
                            data-search="{{ $bclass->equity_class_name }}">{{ $bclass->equity_class_name }}</option>
                    @endforeach
                @endforeach
            </select>
                    ...
                    <div class="d-flex flex-column align-items-center position-relative">
                        ...
                        <div class="w-100">
                            <div class="my-2 p-1 d-flex border border-gray-200 bg-white rounded">
                                        ...
                                        <div x-show="index > 0"
                                            ...
                                            <div class="d-flex flex-auto flex-row-reverse">
                                                <div x-on:click.stop="remove(index, option)"
                                                    class="tag-cross d-flex justify-content-center align-items-center"
                                ...
                                <div
                                    class="text-muted py-1 px-2 border-start d-flex align-items-center border-secondary">
                                    ...
                                    <button type="button" x-show="isOpen()" x-on:click="close()"
                                        class="btn btn-link p-0 text-secondary">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="bi bi-chevron-up"
                                            fill="currentColor" viewBox="0 0 16 16">
                                            <path fill-rule="evenodd"
                                                d="M1.646 11.354a.5.5 0 0 1 .708 0L8 6.707l5.646 5.647a.5.5 0 0 1-.708.708l-6-6a.5.5 0 0 1-.708 0l-6 6a.5.5 0 0 1 0 .708z" />
                                        </svg>
                                    </button>
                                </div>
                            </div>
                        ...
                        <div class="w-100">
                            <div x-show.transition.origin.top="isOpen()" x-trap="isOpen()"
                                class="position-absolute shadow-lg top-100 bg-white z-40 w-100 left-0 rounded max-h-80"
                                ...
                                <div class="d-flex flex-column w-100">
                                    ...
                                    <!-- Options container -->
                                    <ul class="z-10 mt-0 w-100 bg-white shadow-lg max-h-80 rounded py-0 text-base ring-1 ring-black ring-opacity-5 focus:outline-none overflow-y-auto sm:text-sm"
                                        tabindex="-1" role="listbox" @keyup.delete="deselect">
                                        <template x-for="(option,index) in options"
                                            :key="option.text + Math.random().toString(36).substring(2, 9 + 2)">
        ...
        <span x-show="distributionErrors.included_classes" x-text="distributionErrors.included_classes"
            class="text-danger"></span>
    </div>
    {{--  <div class="form-group mb-3">
        <label for="amount">Class A - Limited partners amount <span class="text-danger">*</span></label>
        <input type="number" class="form-control" id="amount" x-model="distributionForm.amount">
    </div>  --}}
    <div class="form-group mb-3">
        <label for="period_start_date">Period start date <span class="text-danger">*</span></label>
        <input type="date" onclick="this.showPicker()" class="form-control" id="period_start_date"
            x-on:input="distributionErrors.start_date =''" x-model="distributionForm.start_date">
        <span x-show="distributionErrors.start_date" x-text="distributionErrors.start_date"
            class="text-danger"></span>
    </div>
    <div class="form-group mb-3">
        <label for="period_end_date">Period end date <span class="text-danger">*</span></label>
        <input type="date" onclick="this.showPicker()" class="form-control" id="period_end_date"
            x-on:input="distributionErrors.end_date =''" x-model="distributionForm.end_date">
    ...
    <div class="form-group mb-3">
        ...
        <input type="date" onclick="this.showPicker()" class="form-control" id="distribution_date"
            x-on:input="distributionErrors.distribution_date =''" x-model="distributionForm.distribution_date">
        <span x-show="distributionErrors.distribution_date" x-text="distributionErrors.distribution_date"
            class="text-danger"></span>
    </div>
    {{--  memo  --}}
    <div class="form-group mb-3">
    ...
    <div>
        <div class="form-check form-check-inline">
            <input class="form-check-input" type="radio" name="visibility" id="visible" :value="1"
                x-model="distributionForm.is_visible">
            <label class="form-check-label" for="visible">Visible</label>
        </div>
        <div class="form-check form-check-inline">
            <input class="form-check-input" type="radio" name="visibility" id="hidden" :value="0"
                x-model="distributionForm.is_visible">
    ...
    <div class="d-flex mt-5 justify-content-between">
        ...
        <button type="submit" class="btn btn-secondary" @click="submitDistributionForm(distributionForm)">Create
            distribution</button>
    </div>
</div>
@push('script')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Activate the 'distribution-waterfalls' tab
            const tabTrigger = document.querySelector('#distribution-waterfalls-tab');
            if (tabTrigger) {
                tabTrigger.click(); // Activate the tab
            }

            // Wait for the tab to activate, then click the Add Waterfall button
            setTimeout(() => {
                const addWaterfallButton = document.querySelector(
                    'button[data-bs-target="#addWaterfallModal"]');
                if (addWaterfallButton) {
                    addWaterfallButton.click(); // Trigger the button
                }
            }, 500); // Delay to ensure tab content is loaded
        });
    </script>
@endpush
