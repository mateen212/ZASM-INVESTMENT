...
class RegisterController extends Controller
{
    ...
    protected function create(array $data)
    {
        ...
        // Check for email in the members table in the database
        $member = Member::where('email_address', $data['email'])->first();
        if ($member && $member->status == 0) {
            // Create new admin user
            
            $adminUser = new Admin();
            $adminUser->email     = strtolower($data['email']);
            $adminUser->name = $member->firstname;
            $adminUser->username  = $member->email_address;
            $adminUser->password  = Hash::make($data['password']);
            $adminUser->save();

            $adminUser->assignRole('Partner');
