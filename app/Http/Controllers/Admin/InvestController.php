...
class InvestController extends Controller
{
    ...
    public function dischargePreview(Request $request, $id)
    {
        $request->validate([
        ...
        $profits  = Profit::pending()->where('property_id', $property->id)->with(['user', 'invest'])->get();

        $totalProfit = 0;
        foreach ($profits as $profit) {
            $propertyInvest = new PropertyInvest($profit->property, $profit->invest, user: $profit->user);
            $profitAmount   = $propertyInvest->calProfit($profit, $request->new_profit_amount, true);
            $totalProfit    += $profitAmount;
        }

        return response()->json([
            'profitsAmount'     => showAmount($profitAmount),
            'totalProfitAmount' => showAmount($totalProfit),
            'totalProfitCount'  => $profits->count(),
            'submitUrl'         => route('admin.invest.profit.discharge', $property->id)
        ]);
    }
